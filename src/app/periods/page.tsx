...
export default async function PeriodsPage() {
  ...
  if (!(await hasPermission("member.view"))) {
    return <DashboardShell accountName={profile.fullName} content={<AccessNotice title="Periode" description="Kelola masa kepengurusan dan histori organisasi." reason="Kamu memerlukan izin member.view. Hubungi Super Admin untuk mengatur role kamu." />} />;
  }
  const [data, canManage] = await Promise.all([loadPeriods(), hasPermission("system.manage")]);
  return <DashboardShell accountName={profile.fullName} content={<PeriodsView {...data} canManage={canManage} />} />;
}
