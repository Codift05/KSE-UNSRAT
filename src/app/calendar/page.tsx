...
export default async function CalendarPage() {
  ...
  if (!(await hasPermission("member.view"))) {
    return <DashboardShell accountName={profile.fullName} content={<AccessNotice title="Kalender" description="Jadwal rapat, program, tenggat, dan kegiatan internal." reason="Kamu memerlukan izin member.view. Hubungi Super Admin untuk mengatur role kamu." />} />;
  }
  const [data, canManage] = await Promise.all([loadCalendar(), hasPermission("program.update")]);
  return <DashboardShell accountName={profile.fullName} content={<CalendarView {...data} canManage={canManage} />} />;
}
