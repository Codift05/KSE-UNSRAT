...
export default async function ManagementPage() {
  ...
  if (!(await hasPermission("member.view"))) {
    return <DashboardShell accountName={profile.fullName} content={<AccessNotice title="Kepengurusan" description="Struktur pengurus pada periode yang sedang aktif." reason="Kamu memerlukan izin member.view. Hubungi Super Admin untuk mengatur role kamu." />} />;
  }
  const [data, canManage] = await Promise.all([loadManagement(), hasPermission("system.manage")]);
  return <DashboardShell accountName={profile.fullName} content={<ManagementView {...data} canManage={canManage} />} />;
}
