...
export default async function DivisionsPage() {
  ...
  if (!(await hasPermission("member.view"))) {
    return <DashboardShell accountName={profile.fullName} accountRole={profile.roleLabel} content={<AccessNotice title="Divisi" description="Pantau koordinator, anggota, dan program setiap divisi." reason="Kamu memerlukan izin member.view. Hubungi Super Admin untuk mengatur role kamu." />} />;
  }
  const [data, canManage] = await Promise.all([loadDivisions(), hasPermission("member.update")]);
  return <DashboardShell accountName={profile.fullName} accountRole={profile.roleLabel} content={<DivisionsView {...data} canManage={canManage} />} />;
}
