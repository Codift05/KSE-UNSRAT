...
export type AccountRow = { id: string; name: string; username: string; email: string; memberStatus: string; access: "Aktif" | "Dinonaktifkan"; createdAt: string };
...
const loadAccountRows = unstable_cache(async (): Promise<AccountRow[]> => {
  const [{ data: users, error: usersError }, { data: profiles, error: profilesError }] = await Promise.all([
    ...
    supabaseAdmin.from("profiles").select("id,full_name,username,member_status").order("full_name").limit(200),
  ]);
  ...
  const rows = (users.users || []).map(user => {
    ...
    return { id: user.id, name: profile?.full_name || String(user.user_metadata.full_name || "Tanpa nama"), username: profile?.username || "-", email: user.email || "-", memberStatus: memberStatusLabel(rawStatus), access, createdAt: new Intl.DateTimeFormat("id-ID", { day: "numeric", month: "short", year: "numeric" }).format(new Date(user.created_at)) };
