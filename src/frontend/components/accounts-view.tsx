...
export function AccountsView({ accounts, currentUserId }: { accounts: AccountRow[]; currentUserId: string }) {
  ...
  return <div className="content accounts-page"><section className="page-heading"><div><h1>Akun beswan</h1><span>Buat dan kelola akses login tanpa membuat data anggota terpisah.</span></div></section>
    <section className="panel account-create"><div className="panel-heading"><div><h2>Buat akun</h2><p>Beswan masuk dengan username. Email boleh dikosongkan; sistem membentuknya sendiri.</p></div></div><form action={createAction}><label>Nama lengkap<input name="full_name" required maxLength={100} /></label><label>Username<input name="username" required minLength={3} maxLength={20} placeholder="ezra" /></label><label>Email (opsional)<input name="email" type="email" placeholder="kosongkan bila tidak ada" /></label><label>Password awal<input name="password" type="password" minLength={8} required /></label><div className="account-submit"><button className="primary-button" type="submit"><Plus size={17} />Buat akun</button></div>{createState.message && <p className="inline-message success">{createState.message}</p>}{createState.error && <p className="inline-message error">{createState.error}</p>}</form></section>
    <section className="panel module-table-panel"><div className="panel-heading module-toolbar"><div><h2>Daftar akun</h2><p>{accounts.length} akun terhubung ke Supabase Auth</p></div><label className="table-search"><MagnifyingGlass size={17} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Cari akun..." aria-label="Cari akun" /></label></div><div className="document-table-wrap"><table className="document-table accounts-table"><thead><tr><th>Beswan</th><th>Username</th><th>Email</th><th>Status anggota</th><th>Akses</th><th>Dibuat</th><th>Kontrol</th></tr></thead><tbody>{rows.map(account => <AccountItem account={account} currentUserId={currentUserId} onSelect={(action) => setSelected({ account, action })} key={account.id} />)}{!rows.length && <tr><td className="empty-table" colSpan={7}>Belum ada akun yang sesuai.</td></tr>}</tbody></table></div></section>
...
function AccountItem({ account, currentUserId, onSelect }: { account: AccountRow; currentUserId: string; onSelect: (action: "reset" | "delete") => void }) {
  ...
  return <tr><td><strong>{account.name}</strong></td><td>{account.username}</td><td>{account.email}</td><td>{account.memberStatus}</td><td><em className={account.access === "Aktif" ? "complete" : "review"}>{account.access}</em></td><td>{account.createdAt}</td><td><div className="account-actions">
